...
const actions = [
  { label: "Buy Prepaid SIM", icon: "/icons/ui/cat-sim.svg" },
  { label: "Buy Postpaid", icon: "/icons/ui/cat-box.svg" },
  { label: "Buy Accesories", icon: "/icons/ui/cat-headphones.svg" },
  { label: "Browse Catalog", icon: "/icons/ui/cat-bag.svg" },
];
      ...
      <div className="flex min-h-0 w-full flex-1 items-start gap-6">
        {actions.map((action) => (
          <div key={action.label} className="flex h-full min-w-0 flex-1 flex-col items-start">
            <button className="flex min-h-0 w-full flex-1 cursor-pointer flex-col items-center justify-center overflow-clip rounded-card bg-surface p-4 hover:bg-[#ececec]">
